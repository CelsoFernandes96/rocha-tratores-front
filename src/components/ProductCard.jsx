...
export default function ProductCard({ product, showCategory }) {
  const installments = product.installments ?? 10
  const installmentValue = product.price / installments

  return (
    <article className={styles.card}>
      <Link to={`/produtos/${product.slug}`} className={styles.imageWrap}>
        <img src={product.image} alt={product.name} />
        {showCategory && (
          <span className={styles.category}>{getCategoryName(product.category)}</span>
        )}
      </Link>
      <div className={styles.body}>
        ...
        <div className={styles.actions}>
          ...
          <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className={styles.detailBtn}>
            <WhatsAppIcon />
            Saiba mais
          </a>
        </div>
